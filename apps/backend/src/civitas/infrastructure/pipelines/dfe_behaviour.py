...
from .contracts import dfe_behaviour as behaviour_contract
...
BRONZE_MANIFEST_FILE_NAME = "dfe_behaviour.manifest.json"
...
class DfeBehaviourPipeline:
    ...
    def __init__(
        ...
        engine: Engine | None,
        *,
        publication_slug: str = "suspensions-and-permanent-exclusions-in-england",
        ...
        if lookback_years <= 0:
            raise ValueError("DfE behaviour lookback years must be greater than 0.")
    ...
    def download(self, context: PipelineRunContext) -> int:
        ...
        manifest_payload = {
            "downloaded_at": datetime.now(timezone.utc).isoformat(),
            "normalization_contract_version": behaviour_contract.CONTRACT_VERSION,
    ...
    def stage(self, context: PipelineRunContext) -> StageResult:
        ...
        if len(manifest_assets) == 0:
            raise FileNotFoundError(
                f"DfE behaviour manifest not found at '{manifest_path}'. Run download stage first."
            )
            ...
            with csv_path.open("r", encoding="utf-8-sig", newline="") as file_handle:
                ...
                for raw_row in reader:
                    if not _is_school_level_row(raw_row):
                        continue
                    normalized_row, rejection = behaviour_contract.normalize_row(
                        raw_row,
                        release_slug=asset.release_slug,
                        release_version_id=asset.release_version_id,
                        file_id=asset.file_id,
                    ...
                    staged_row = _StagedRow(
                        ...
                        suspensions_count=normalized_row["suspensions_count"],
                        suspensions_rate=normalized_row["suspensions_rate"],
                        permanent_exclusions_count=normalized_row["permanent_exclusions_count"],
                        permanent_exclusions_rate=normalized_row["permanent_exclusions_rate"],
                        ...
                        [
                            {
                                ...
                                "suspensions_count": row.suspensions_count,
                                "suspensions_rate": row.suspensions_rate,
                                "permanent_exclusions_count": row.permanent_exclusions_count,
                                "permanent_exclusions_rate": row.permanent_exclusions_rate,
        ...
        return StageResult(
            ...
            contract_version=behaviour_contract.CONTRACT_VERSION,
        )
                    ...
                    text(
                        ...
                        WITH upserted AS (
                            ...
                            SELECT
                                ...
                                staged.suspensions_count,
                                staged.suspensions_rate,
                                staged.permanent_exclusions_count,
                                staged.permanent_exclusions_rate,
    ...
    def _discover_release_file(self, *, release_slug: str) -> _ReleaseFileDescriptor:
        ...
        try:
            ...
            raise ValueError(
                "Unable to resolve behaviour release page for "
        ...
        selected = _select_behaviour_school_level_file(download_files)
        if selected is None:
            raise ValueError(
                f"No school-level behaviour file found for {self._publication_slug}/{release_slug}."
            )
        ...
        return _ReleaseFileDescriptor(
            ...
            file_id=_required_str(selected.get("id"), "Selected behaviour file has no id."),
            file_name=_required_str(selected.get("name"), "Selected behaviour file has no name."),
        )

    @staticmethod
    def _staging_table_name(context: PipelineRunContext) -> str:
        return f"dfe_behaviour__{context.run_id.hex}"
...
def _select_behaviour_school_level_file(
    ...
    for item in download_files:
        ...
        if "school level" not in normalized_name:
            continue
        if "suspensions" not in normalized_name and "exclusions" not in normalized_name:
            continue
        if "alternative provision" in normalized_name:
            continue
        if "by term" in normalized_name:
            continue
...
def _build_bronze_file_name(descriptor: _ReleaseFileDescriptor) -> str:
    ...
    return f"dfe_behaviour_{release_slug_token}_{descriptor.file_id}.csv"
