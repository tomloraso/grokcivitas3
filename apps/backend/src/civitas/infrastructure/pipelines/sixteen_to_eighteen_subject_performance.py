...
from .contracts import (
    sixteen_to_eighteen_subject_performance as subject_contract,
)

BRONZE_MANIFEST_FILE_NAME = "sixteen_to_eighteen_subject_performance.manifest.json"
...
class SixteenToEighteenSubjectPerformancePipeline:
    ...
    def stage(self, context: PipelineRunContext) -> StageResult:
        ...
        if not manifest_assets:
            raise FileNotFoundError(
                "16 to 18 subject performance manifest is missing; run download first."
            )
        ...
        for asset in manifest_assets:
            ...
            if not csv_path.exists():
                raise FileNotFoundError(
                    f"16 to 18 subject performance bronze file not found at '{csv_path}'."
                )
            with csv_path.open("r", encoding="utf-8-sig", newline="") as file_handle:
                reader = csv.DictReader(file_handle)
                if reader.fieldnames is None:
                    raise ValueError(
                        f"16 to 18 subject performance CSV '{csv_path}' has no header row."
                    )
                ...
                for raw_row in reader:
                    ...
                    staged_row = _StagedRow(
                        ...
                        exam_cohort=normalized_row["exam_cohort"],
                        qualification_detailed=normalized_row["qualification_detailed"],
                        qualification_family=normalized_row["qualification_family"],
                        qualification_level=normalized_row["qualification_level"],
                        a_level_equivalent_size=normalized_row["a_level_equivalent_size"],
                        gcse_equivalent_size=normalized_row["gcse_equivalent_size"],
                        grade_structure=normalized_row["grade_structure"],
                        subject=normalized_row["subject"],
                        grade=normalized_row["grade"],
                        entries_count=normalized_row["entries_count"],
    ...
    def promote(self, context: PipelineRunContext) -> int:
        ...
        with self._engine.begin() as connection:
            ...
            final_rows: dict[tuple[str, ...], Mapping[str, object]] = {}
            ...
            for staged_row in staged_rows:
                ...
                final_rows[_promote_key(mapped_row)] = mapped_row
            ...
            rows_to_upsert = [
                {
                    "urn": str(row["school_urn"]),
                    "academic_year": str(row["academic_year"]),
                    "school_laestab": row["school_laestab"],
                    "school_name": row["school_name"],
                    "old_la_code": row["old_la_code"],
                    "new_la_code": row["new_la_code"],
                    "la_name": row["la_name"],
                    "source_version": row["source_version"],
                    "source_downloaded_at_utc": row["source_downloaded_at_utc"],
                    "exam_cohort": row["exam_cohort"],
                    "qualification_detailed": row["qualification_detailed"],
                    "qualification_family": row["qualification_family"],
                    "qualification_level": row["qualification_level"],
                    "a_level_equivalent_size": row["a_level_equivalent_size"],
                    "gcse_equivalent_size": row["gcse_equivalent_size"],
                    "grade_structure": row["grade_structure"],
                    "subject": row["subject"],
                    "grade": row["grade"],
            ...
            if rows_to_upsert:
                upsert = text(
                    ...
                    ON CONFLICT (
                        ...
                        a_level_equivalent_size = excluded.a_level_equivalent_size,
                        gcse_equivalent_size = excluded.gcse_equivalent_size,
                ...
                for row_chunk in chunked(rows_to_upsert, context.promote_chunk_size):
    ...
    def _staging_table_name(context: PipelineRunContext) -> str:
        return f"school_16_to_18_subject_results__{context.run_id.hex}"
        ...
        connection.execute(
            text(
                ...
                CREATE TABLE staging.{table_name} (
                    ...
                    a_level_equivalent_size double precision NULL,
                    gcse_equivalent_size double precision NULL,
...
def _promote_key(row: Mapping[str, object]) -> tuple[str, ...]:
    return (
        ...
        str(row["exam_cohort"]),
        str(row["qualification_detailed"]),
        str(row["subject"]),
        str(row["grade"]),
...
def _download_payload(*, source_reference: str, timeout_seconds: float) -> _DownloadedPayload:
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        with urllib.request.urlopen(request, timeout=timeout_seconds) as response:
            ...
            if observed_content_type is None or "csv" not in observed_content_type.casefold():
                raise ValueError(
                    "16 to 18 subject performance source returned a non-CSV response "
    ...
    if not source_path.exists():
        raise FileNotFoundError(
            f"Configured 16 to 18 subject performance source '{source_path}' was not found."
        )
    return _DownloadedPayload(
...
def _inspect_downloaded_csv(
    *,
    ...
    if not body.strip():
        raise ValueError("16 to 18 subject performance download returned an empty payload.")
    content = body.decode("utf-8-sig")
    reader = csv.DictReader(content.splitlines())
    if reader.fieldnames is None:
        raise ValueError("16 to 18 subject performance CSV has no header row.")
    headers = tuple(reader.fieldnames)
    subject_contract.validate_headers(headers)
    rows = list(reader)
    if not rows:
        raise ValueError("16 to 18 subject performance CSV has no data rows.")
...
def _derive_academic_year(rows: list[dict[str, str]]) -> str:
    ...
    if len(academic_years) != 1:
        raise ValueError("16 to 18 subject performance CSV must contain exactly one academic year.")
...
def _resolve_bronze_file_name(*, observed_file_name: str | None, academic_year: str) -> str:
    ...
    if file_name is not None:
        return file_name
    return f"sixteen_to_eighteen_subject_performance_{academic_year.replace('/', '')}.csv"
