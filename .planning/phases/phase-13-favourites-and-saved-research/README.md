# Phase 13 Design Index - Favourites And Saved Research
...
Phase 13 exists because favourites are valuable, but they are not required to prove the first premium launch. The launch bundle for Phase 10 therefore stays focused on identity, entitlements, checkout, AI analyst, compare, and neighbourhood gating. Favourites can be pulled forward only if launch blockers are already closed and capacity remains.

## Solution Grounding

Phase 13 should extend the solution that already exists in the repo, not invent a parallel account stack.
...
- Phase 10 already defines the capability-based access model. If favourites are later treated as paid, Phase 13 should consume that access slice and capability registry rather than add bespoke entitlement logic.
...
Phase 13 is split into three deliverables:

1. `13A-account-favourites-foundation.md`
2. `13B-favourites-web-and-library.md`
3. `13C-phase-13-quality-gates.md`

## Execution Sequence

1. Complete `13A` first to freeze the domain model, access policy, and persistence contract.
2. Complete `13B` once the backend contract is agreed so search, profile, header, and account-library UI stay aligned.
3. Complete `13C` as final quality closeout and sign-off.
...
- 2026-03-10: Phase 13 implemented and committed (`fec7d4f`). SaveSchoolButton with Heart icon, tooltip, status-aware labels. Backend favourites API with `saved_schools` table. Mapper compat fix for undefined `saved_state` on search results (`favourites/mappers.ts`).
