# Phase 14 / 14D Design - Quality Gates
...
2. Silver rejects rows with invalid URNs or missing school names.
3. Blank, suppressed, or unpublished numeric cells are normalized to `null`; they are not treated as zero and do not by themselves reject the row.
4. Gold derived metrics do not divide by zero.
5. Finance row counts and populated-metric ratios are captured in operational validation.
...
3. Profile and trends API contracts are updated in the same PR as persistence changes, including finance completeness fields.
4. Benchmark materialization is rerun manually after promote and validated against sample schools.
5. Phase notes remain explicit that compare metric exposure is deferred even though finance benchmark keys are registered.
